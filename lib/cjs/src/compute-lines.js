 ...
 */
const computeLineInformation = (oldString, newString, disableWordDiff = false, lineCompareMethod = DiffMethod.CHARS, linesOffset = 0, showLines = [], lineDiffMethod = null) => {
    let diffArray = [];
    // Use diffLines for strings, and diffJson for objects...
    if (lineDiffMethod) {
        diffArray = lineDiffMethod(oldString, newString);
    }
